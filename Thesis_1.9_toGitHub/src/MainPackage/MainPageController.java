...
import static MainPackage.Main.cssLoc;
...
public class MainPageController {
    ...
    static boolean isZoomReady = false;
    static boolean isMoveReady = false;
    ...
    private static Text currentQuality;
    private static Text renderedPictures;
    private static Text coordXText;
    private static Text coordYText;
    private static Rectangle currentDrawColor;
    ...
    private BorderPane createLayout(){
        ...
        btnNewProject.setOnAction(actionEvent -> {
            if (sectionGeneral.getValue("PANORAMIC_VIEWER_PATH") == null){
                Parent root;
                try {
                    root = FXMLLoader.load(getClass().getResource("InstallViewer.fxml"));
        ...
        btnLoadProject.setOnAction(actionEvent -> {
            Parent root;
            try {
                root = FXMLLoader.load(getClass().getResource("LoadProject.fxml"));
                Scene scene = new Scene(root, 300, 200);
                scene.getStylesheets().add(cssLoc);
        ...
        singlePicture.setOnAction(actionEvent -> {
            ...
            sc.setScreen(singlePicture.getScene(), "MainPackage/SinglePictureRender.fxml");
        });
        ...
        VBox vBox = new VBox();
        vBox.getChildren().addAll(progressTab, progressBar);
        ...
        finishedButton.setOnAction(actionEvent -> {
            constructGridViewer();
            PictureRenderer.run();
            isMoveReady = true;
        });
    ...
    private static void constructGridViewer(){
        ...
        fullPictView.setOnAction(actionEvent -> constructLowQualityViewer());*/
        ...
        currentQuality = new Text();
        currentQuality.getStyleClass().add("Custom-Text2");

        Text sizeOfViewedArea = new Text();
        sizeOfViewedArea.getStyleClass().add("Custom-Text2");
        sizeOfViewedArea.setText("Size of area: " + 1024*1024*4 + " pixels");

        renderedPictures = new Text();
        renderedPictures.getStyleClass().add("Custom-Text2");

        coordXText = new Text();
        coordXText.getStyleClass().add("Custom-Text2");

        coordYText = new Text("Coordinate Y: 120");
        coordYText.getStyleClass().add("Custom-Text2");
        Text currentColorText = new Text("Current draw color: ");
        currentColorText.getStyleClass().add("Custom-Text2");

        currentDrawColor = new Rectangle(30,30);
        currentDrawColor.getStyleClass().add("Custom-Rectangle");

        HBox rectBox = new HBox();
        rectBox.getChildren().addAll(currentColorText, currentDrawColor);
        rectBox.setAlignment(Pos.CENTER_LEFT);

        File mrxsFile = new File(projectFolder + "\\" + sectionMainPane.getValue("MRXS_FILE_NAME"));
        ImageView imageView = new ImageView();
        try {
            Image mrxsImage = new Image(mrxsFile.toURI().toURL().toString());
            imageView.setImage(mrxsImage);
            Image croppedImage = cropImage(imageView);
            imageView.setImage(croppedImage);
            boolean isWidth = croppedImage.getWidth() > croppedImage.getHeight();
            if (isWidth){
                imageView.setFitWidth(300);
            }
            else imageView.setFitHeight(450);
        ...
        infoBox.getChildren().addAll(currentQuality, sizeOfViewedArea, renderedPictures, coordXText, coordYText, rectBox, imageView);
        infoBox.setAlignment(Pos.TOP_LEFT);
        infoBox.setSpacing(40);
        infoBox.setPrefWidth(500);
        infoBox.setPrefHeight(1000);
        infoBox.getStyleClass().add("Custom-Menu2");
        infoBox.setPadding(new Insets(20));

        VBox containerBox = new VBox();
        containerBox.getChildren().add(infoBox);
        containerBox.setPadding(new Insets(20));


        main.setCenter(null);
        main.setTop(main.getTop());
        main.setLeft(vBox1);
        main.setRight(containerBox);
        isZoomReady = true;
    }

    private static void makeGridViewer() {
        Stage gridViewer = new Stage();
        gridViewer.initStyle(StageStyle.TRANSPARENT);
        gridViewer.initModality(Modality.NONE);
        gridViewer.initOwner(pStage);

        StackPane stackPane = new StackPane();
        stackPane.getChildren().add(grid);
        stackPane.getStyleClass().add("Custom-BorderOnly");
        stackPane.setOnScroll(event -> {
            if (isZoomReady){
                if (event.getDeltaY() < 0){
                    //Zoom out
                    if (grid.getScaleX() > 1){
                        grid.setScaleX(grid.getScaleX() / 2);
                        grid.setScaleY(grid.getScaleX());
                    }
                }
                else {
                    //Zoom in
                    if (grid.getScaleX() < 100){
                        grid.setScaleX(grid.getScaleX() * 2);
                        grid.setScaleY(grid.getScaleX());
                    }
                }
        ...
        stackPane.setOnMousePressed(event -> {
            if (isZoomReady) {
                ...
                orgTranslateX = grid.getTranslateX();
                orgTranslateY = grid.getTranslateY();
            }
        });
        stackPane.setOnMouseDragged(event -> {
            if (isZoomReady) {
                ...
                grid.setTranslateX(newTranslateX);
                grid.setTranslateY(newTranslateY);
            }
        });
        stackPane.setOnKeyReleased(MainPageController::handleKeyPressed);
        ...
        gridViewer.setX(300);
        gridViewer.setY(70);
        gridViewer.setScene(scene);
        gridViewer.sizeToScene();
        gridViewer.show();
    }

    private static void setOnAction(ToggleButton btn, VBox vBox, Constant qualityLevel) {
     ...
     */
    public void handleFxmlKeyPress(KeyEvent keyEvent){
    ...
    static void setSizeText(){
        switch (qualityLvl) {
            case QUALITY_LEVEL_RAWIMAGE:
                currentQuality.setText("Quality: " + "2048X2048" + " pixels/image");
                renderedPictures.setText("Number of rendered images: " + 4);
                break;
            case QUALITY_LEVEL_1024:
                currentQuality.setText("Quality: " + "1024X1024" + " pixels/image");
                renderedPictures.setText("Number of rendered images: " + 16);
                break;
            case QUALITY_LEVEL_512:
                currentQuality.setText("Quality: " + "512X512" + " pixels/image");
                renderedPictures.setText("Number of rendered images: " + 64);
                break;
            case QUALITY_LEVEL_256:
                currentQuality.setText("Quality: " + "256X256" + " pixels/image");
                renderedPictures.setText("Number of rendered images: " + 256);
                break;
            case QUALITY_LEVEL_128:
                currentQuality.setText("Quality: " + "128X128" + " pixels/image");
                renderedPictures.setText("Number of rendered images: " + 1024);
                break;
        }
    }

    static void setCoordText(){
        int x = position1.get(0).getCoordX();
        int y = position1.get(0).getCoordY();
        coordXText.setText("Coordinate X: " + x);
        coordYText.setText("Coordinate Y: " + y);
    }

    public static void setDrawColor(int color){
        // TODO : To be implemented
        //currentDrawColor.getStyleClass().add()
    }

    @NotNull
    static Button makeCancelButton(Stage currentStage) {
        Button cancel = new Button();
        cancel.setText(sectionGeneral.getValue("CANCEL_BUTTON_TEXT"));
        cancel.setMinSize(Button.USE_PREF_SIZE, Button.USE_PREF_SIZE);
        cancel.getStyleClass().add("Custom-Button1");
        cancel.setOnAction(actionEvent -> currentStage.close());
        return cancel;
    }

    @NotNull
    private static Image cropImage(@NotNull ImageView imageView){
        Image image = imageView.getImage();
        PixelReader pixelReader = image.getPixelReader();
        boolean hasColor = false;
        boolean topFound = false;
        int start =(int) image.getWidth();
        int end = -1;
        ...
        for (int y = 0; y < image.getHeight(); y++) {
            for (int x = 0; x < image.getWidth(); x++) {
                ...
                if (pixelColor.getRed() != 0.0 || pixelColor.getGreen() != 0.0 || pixelColor.getBlue() != 0.0){
                    if (x < start){
                        start = x;
                    }
                    if (x > end){
            ...
            if (hasColor && !topFound){
                top = y;
                topFound = true;
            }
            else if (topFound && !hasColor){
                bottom = y;
                topFound = false;
            }
            hasColor = false;
        ...
        left = start;
        right = end;
        Rectangle2D rect= new Rectangle2D(left, top, right - left, bottom - top);
        SnapshotParameters snapshotParameters = new SnapshotParameters();
        snapshotParameters.setViewport(rect);
