...
public class LoadProjectController{
    ...
    private static BorderPane makeWindow() {
        ...
        EventHandler<ActionEvent> load = event -> {
            ...
            if (!RobotFunctions.runFileScan()){
                showErrorText(true,sectionLoadProject.getValue("ERROR_TEXT"));
            }
            else {
                MainPageController.constructProjectText(((Button)event.getTarget()).getText());
                showErrorText(false,sectionLoadProject.getValue("ERROR_TEXT"));
        ...
        if (projects.exists()) {
            ...
            showErrorText(true,"No projects");
        }
        ...
        Button cancelButton = makeCancelButton(dialogLoad);
     ...
     */
    private static void showErrorText(boolean condition, String errortext){
