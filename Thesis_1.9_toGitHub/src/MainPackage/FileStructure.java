...
class FileStructure {
    ...
    private static final String snapExtension = ".jpg";
    ...
    @NotNull
    @Contract("_ -> new")
    static File makeFile(@NotNull SnappedImage snappedImage){
        ...
        assert secondaryFile != null;
        return new File(pathFormatter(secondaryFile.getPath(),folderOneName,folderTwoName,sequenceID + end + snapExtension));
    }

    @NotNull
    @Contract("_ -> new")
    static File makeCanvasFile(@NotNull File snappedFile){
        return new File(snappedFile.getParent() + "\\MASK_" + snappedFile.getName());
    }
